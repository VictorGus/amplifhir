...
(deftest crud-patch
  (testing "Edit patient test"
    (db/execute {:insert-into :patient
                 :values [{:id "pt-1"
                           :resource (hsql/call :cast (json/generate-string {:identifier "123"}) :jsonb)}]} db/test-config)
    (let [resp (-> (-> (mock/request :put "/Patient/pt-1")
                       (mock/json-body {:identifier "1234"
                                        :name [{:family "Test"}]})) app*)]
      (matcho/match {:id "pt-1"
                     :resource {:name [{:family "Test"}]
                                :identifier "1234"}}
                    (-> (mock/request :get "/Patient/pt-1") app*
                        :body
                        (json/parse-string true)
                        :entry))
      ...
      (db/execute {:delete-from :patient
                   :where [:= :id "pt-1"]} db/test-config)

      (is (= 201 (:status (-> (-> (mock/request :put "/Patient/pt-1")
                                  (mock/json-body {:identifier "1234"
                                                   :name [{:family "Test"}]})) app*))))
